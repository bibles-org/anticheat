...
namespace utils {
    ...
    void submit_screenshot_report(const std::string& reason) {
        ...
        loader::append_report(
                message_id::screenshot_error, reason.c_str(), reason.size(), dxgi_error_tag.c_str(),
                dxgi_error_tag.size(), nullptr, 0
        );
        ...
        if (!buffer.empty()) {
            ...
            return loader::append_report(
                    message_id::screenshot, blt_reason.c_str(), blt_reason.size(), nullptr, 0, buffer.data(),
                    buffer.size()
            );
        }
